...
const CourseStudy = ({ user }) => {
  ...
  // We use useCallback to create a stable version of fetchCourse
  // This prevents the useEffect from running in an infinite loop.
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        await stableFetchCourse(courseId); // Use the stable function
    ...
    fetchAllData();
  }, [courseId, user, navigate, stableFetchCourse]); // Dependency array is now safe
    ...
    <div className="course-study-page-container">
      {course && (
        <div className="course-study-card">
          ...
          {/* Use course.image.url directly without the server prefix */}
          <img
